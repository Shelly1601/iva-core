...
assert.equal(macWakeGuardPolicy().displaySleepMinimumIdleSeconds, 60);
...
assert.equal(pipedriveStatus.applePasswordsDatabaseVerified, false);
assert.equal(brokerStatus.services.find(row=>row.id==='panasonic').keychainReady, false);
assert.equal(brokerStatus.services.find(row=>row.id==='panasonic').hasStoredCredentials, false);

...
try {
  ...
  assert.equal(deviceCommandNeedsImmediateUiLock('knowledge.import.start'), true, 'Kursanmeldung wird sichtbar und ausschließlich unter der rechten UI-Sperre geprüft');
  ...
  const imacMetadata = {
    ...
    allowedActions: ['agent.status', 'computer.status', 'funding.monitor.status', 'funding.legacy-monitor.suspend', 'project.workflow.run', 'planbar.search.refresh', 'planbar.customer.schedule', 'portal.login', 'portal.credentials.status', 'knowledge.import.start', 'codex.task.start', 'codex.task.status', 'app.open'],
    credentialEnvelope: { version: 1, algorithm: 'RSA-OAEP-256+A256GCM', publicKey: 'QUJD', fingerprint: '0123456789abcdef01234567' },
  };

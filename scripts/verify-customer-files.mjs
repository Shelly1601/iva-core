...
assert.match(customerHtml, /id="documentInput"[^>]*multiple/);
assert.match(customerHtml, /Dateien hierher ziehen/);
assert.match(customerJs, /id="addDocumentInline"/);
assert.match(customerJs, /documentDropzone\.addEventListener\('drop'/);
assert.match(customerJs, /className = 'quick-delete'/);
assert.match(customerJs, /quickDeleteLocalCustomer\(customer\)/);
assert.match(customerJs, /customer\.source === 'iva' && customer\.workspace\?\.id/);

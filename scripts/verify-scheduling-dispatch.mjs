...
// Nur der Werkzeugtransport ändert sich; Freigaben/Schutzgrenzen nicht.
for (const mode of ['operational', 'project-workflow', 'build']) {
  ...
  if (mode !== 'build') assert.equal(args[args.indexOf('--disable') + 1], 'code_mode_host');
  assert.equal(args.filter(arg => arg === '--add-dir').length, 2);
  assert.equal(args[args.indexOf('-C') + 1], process.env.IVA_DEVICE_WORKSPACE);
  assert.ok(!args.some(arg => /--dangerously-|--ignore-rules|--ignore-user-config|--yolo|danger-full-access/.test(arg)));
}

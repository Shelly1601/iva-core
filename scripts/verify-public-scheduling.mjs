...
assert.equal(unavailable.status,'unavailable','Fehlgeschlagene Prüfung endet statt endlos zu warten');
...
assert.equal((await lateVisitor.availability(lateVisitor.issueToken())).status,'refresh_required','Später hinzugekommene Besucher erhalten automatisch eine neue Leseprüfung');
...
assert(js.includes("result.status === 'refresh_required'")&&js.includes('availableUntil - 10_000'),'Späte Besucher und tatsächliches Ablaufdatum werden berücksichtigt');

...
export function deviceControlSkill({ enqueueDeviceCommand, deviceCommandStatus, listAgentRuns }) {
  return {
    sendCommandToImac: tool({
      description: 'Sendet auf Nadines ausdrücklichen Wunsch einen freigegebenen Befehl an ihren exklusiven Mac Mini. Der Mac Mini holt den Befehl ausgehend ab. Unterstützt Statusprüfung, einen gesperrten Fördermonitor-Prüflauf, Review-Übersicht und das Öffnen freigegebener Apps.',
      parameters: z.object({
        ...
        confirmed: z.boolean().describe('true wenn Nadine die konkrete Aktion beauftragt hat; Mac Mini ist dauerhaft der Standard, keine zweite Ortsfreigabe verlangen'),
        requestText: z.string().max(500).optional(),
      }),
      execute: async ({ action, app, confirmed, requestText }) => {
        if (!confirmed) return { queued: false, error: 'Bitte den Mac Mini-Befehl zuerst ausdrücklich bestätigen.' };
    ...
    runTaskOnImac: tool({
      description: 'Führt eine von Nadine beauftragte lokale Aktion über Codex auf dem exklusiven Mac Mini aus, wenn keine engere Geräteaktion passt. Für IVA-Code-, App- oder Systemänderungen ist startIvaBuild zu verwenden. Die Aktion arbeitet im lokalen IVA-Workspace dieses Mac Mini; Erfolg wird erst nach Prüfung des tatsächlichen Ergebnisses behauptet.',
      parameters: z.object({
        title: z.string().min(3).max(180),
        request: z.string().min(10).max(12_000),
        acceptanceCriteria: z.array(z.string().min(3).max(500)).max(12).optional(),
        confirmed: z.boolean().describe('true wenn Nadine diesen konkreten Auftrag erteilt hat; Mac Mini-Ausführung ist dauerhaft freigegeben'),
      }),
      execute: async ({ title, request, acceptanceCriteria = [], confirmed }) => {
        if (!confirmed) return { queued: false, error: 'Bitte die Ausführung auf dem Mac Mini zuerst ausdrücklich bestätigen.' };
    ...
    getImacCommandStatus: tool({
      description: 'Prüft den Status eines zuvor an Nadines Mac Mini gesendeten Befehls.',
    ...
    getImacTaskStatus: tool({
      description: 'Prüft den tatsächlichen Ausführungsstatus eines lokalen operativen Mac Mini-Codex-Auftrags anhand seiner jobId. Nur status completed zusammen mit resultPreview belegt ein fertiges Ergebnis; queued oder running belegen ausschließlich, dass der Auftrag noch läuft.',
      parameters: z.object({ jobId: z.string().uuid() }),
      execute: async ({ jobId }) => {
        if (typeof listAgentRuns !== 'function') return { found: false, jobId, error: 'Die Mac Mini-Auftragsstatusquelle ist nicht verbunden.' };

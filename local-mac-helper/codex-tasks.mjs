...
Nach dem verifizierten Erstlauf nur neue/geänderte Deals und Quellen bearbeiten. Für bereits vorhandene Deals neue/geänderte Mails, sämtliche neuen oder editierten relevanten Notizen, Dokumente/TMB/Angebote und Fachfelder gegen den gespeicherten Prüfstand vergleichen; unveränderte Akten nicht erneut vollständig lesen. Auch geprüfte unvollständige Fälle mit Quellenstand und offenen Punkten dauerhaft merken; ohne Änderung nur tatsächlich fällige Folgeschritte ausführen. Ein fehlender Vollständigkeitscache ist bei einem belegten offenen Fall kein Auftrag zur täglichen Wiederholung aller PDFs. Eigene bestätigte Aktionen nicht als neue Arbeit auffassen. Fortschritt der gesamten Kette anhand Dealprüfung, verifizierter Übergabe, Fördersummennotiz, WhatsApp an Viktoria und Tabelleneintrag ausweisen; keine Folgeschritte auslassen oder wiederholen.`
    : '';
...
const PROJECT_WORKFLOW_TASKS = Object.freeze({
  ...
  'funding-daily-sequence': Object.freeze({
    ...
    prompt: 'Lies FUNDING_WORKFLOWS.md vollständig. Führe auf diesem Mac Mini die Schritte Vollständigkeit & Unterlagen → Förderhöhe → KfW-Zusagen geordnet aus. Verarbeite ausschließlich neue sowie dauerhaft vorgemerkte noch nicht vollständig bearbeitete E-Mails. Kein täglicher Rücklauf ab August, kein erneutes Lesen bereits erledigter Mails. Nach dem vollständig protokollierten Erstlauf nur neue Deals und relevante Änderungen an bestehenden Deals, Mails, Notizen, Dateien und Feldern bearbeiten; keine tägliche vollständige Aktenlektüre unveränderter Deals. Gespeicherte offene Fälle nur bei neuer Information oder einem konkret fälligen Schritt fortsetzen. Neue KfW-Zusagen und belegte unbeantwortete Anforderungen nach sieben Tagen prüfen. Ein gelesener Posteingang-Eintrag ist noch kein Bearbeitungsnachweis.',
  ...
  'funding-monitor': Object.freeze({
    ...
    prompt: 'Lies FUNDING_WORKFLOWS.md vollständig. Führe ausschließlich Schritt 1 genau einmal auf diesem Mac Mini aus. Neue oder geänderte Mails und Deals anhand des dauerhaften Intakes und Deal-Prüfstands bearbeiten; unveränderte bereits geprüfte offene Fälle nicht erneut vollständig lesen. Nur neue Informationen, noch nicht verifizierte Aktionen oder die tatsächlich fällige Sieben-Tage-Reaktion abarbeiten. Bereits erledigte Mails nicht erneut lesen; niemals die Historie ab August neu scannen.',
  ...
  'kfw-funding-amount-morning': Object.freeze({
    ...
    prompt: 'Lies FUNDING_WORKFLOWS.md vollständig und führe ausschließlich „Förderung 2 – Förderhöhe prüfen“ genau einmal auf diesem Mac Mini aus. Prüfe ausschließlich die bereits vollständig belegten Übergaben aus Schritt 1 und offene eigene Übergaben aus list-pipedrive-funding-handoffs. Keine Förderhöhen-Notiz in unvollständigen Deals und keine Neuanlage allein wegen einer bereits bestehenden Phase Förderung beantragen. Prüfe die vollständige Dealakte einschließlich menschlicher Notizen und verwende den versionierten KfW-Rechenkern für die kurze voraussichtliche Förderschätzung innerhalb dieser Übergabe. Das unterschriebene Angebot ist die Kostenbasis; fehlende BzA-Bestätigung oder fehlendes Antragsdatum blockieren diese Übersicht nicht. Ohne Antragsdatum den aktuellen geprüften Regelstand intern annehmen. Bekannte Grundförderung ausweisen und nur tatsächlich ungeklärte persönliche Boni offen lassen. Keine Quellen-, Datei-, Seiten- oder Regelstandblöcke in der Notiz. Offene Kundenfragen gemäß geprüftem Förder-Versandablauf an den Kunden mit VP im CC bearbeiten; nur bei eindeutigen Quellen und bekanntem Muster versenden, sonst als Entwurf behalten.',
    acceptanceCriteria: ['Die vorläufige Schätzung verwendet belegte Angebotskosten und Wohnungszahl; fehlendes Antragsdatum oder fehlende BzA blockieren sie nicht.', 'MFH-Berechnungen verwenden die korrekte Kostenstaffel und zeigen nach Voraussichtlich den Eurobetrag und Förderung.', 'Neue Förderhöhen-Notizen entstehen nur nach vollständigem Dokumentreview und bestätigtem Wechsel von Antrag/Auftrag eingereicht nach Förderung beantragen über complete-pipedrive-funding-handoff. Sie beginnen mit Voraussichtlich, danach EFH-Prozent beziehungsweise MFH-Eurobetrag und Förderung, dann knapp die Bestandteile und Erklärung; Abschluss exakt (Notiz von Nadine).', 'Fördermails haben eine aktuelle Inhalts- und Empfängerprüfung sowie einen dauerhaften Versandbeleg.'],
  }),
...
export async function startProjectWorkflowTask({
  ...
  if (FUNDING_WORKFLOW_STEPS[normalizedWorkflowId]) {
    ...
    taskDefinition = { ...taskDefinition, prompt: `${taskDefinition.prompt}\n\nVerbindliche Regeln von Nadine: Jeden geprüften offenen Deal sofort auf vier gespeicherte Pflichtangaben prüfen: Telefonnummer und Kunden-E-Mail aus der TMB, Anlage und Auftragsnummer aus dem unterschriebenen Angebot. Im aktuellen Schema liegen Kontaktdaten am verknüpften Kundenkontakt; Anlage und Auftragsnummer in Deal-Feldern. apply-pipedrive-fields verwendet die kanonische Feldquelle. Titel-, Notiz- und Dokumentwerte allein zählen nicht als befüllte Felder. Lücken auch in fortgeschrittenen Deals sofort nachtragen und rücklesen, nicht erst bei KfW-Zusage. Ohne vier bestätigte Werte kein Vollständigkeitsabschluss oder Vorwärts-Phasenwechsel; einen solchen Einzelfall als partial statt completed protokollieren. Bestehende Widersprüche nicht überschreiben. Mails an den Kunden, Vertriebspartner im CC, ausschließlich mit verifizierten Empfängern und bekanntem Muster; bei Unklarheit als Entwurf behalten. Nach sieben vollen Tagen ohne Antwort dieselbe Anforderung einmal an den zuständigen Vertriebsleiter weiterleiten: EKD Katharina Bolz (k.bolz@heat-hero.com), Direktvertrieb Noah Zielinski, Sol Living/Sol Heat Patrick Germer; Adressen aus der geprüften Zuordnung verwenden. Keine geratenen CC-Adressen. Vor jeder Fördermail FUNDING_SEND_STATE.md lesen: funding-send prepare, vollständige aktuelle Entwurfsprüfung, funding-send before-submit, nur bei maySend:true unmittelbar genau einmal senden, danach funding-send complete mit echtem Gesendet-Nachweis. Bei Unterbrechung funding-send resume; niemals einen neuen Vorgang zur Umgehung eines offenen Versandversuchs anlegen. Notizen: kurze Zusammenfassung in Zeile 1, danach nur wesentliche Fakten und echte offene Angaben, exakter letzter Text (Notiz von Nadine). Förderhöhen-Notizen ausschließlich bei vollständigen, lesbaren und im Deal rückgelesenen Unterlagen gemeinsam mit dem bestätigten Wechsel von Auftrag/Antrag eingereicht nach Förderung beantragen anlegen: complete-pipedrive-funding-handoff /pfad/uebergabe.json --commit. Vorher keine Förderhöhen-, Zuschussbetrags- oder Förderhöhe-offen-Notiz, sondern nur Unterlagen vervollständigen. Die neue Fördernotiz wird erst nach dem bestätigten Phasenwechsel gespeichert. Bereits verschobene Deals nicht pauschal neu benoten; nur eigene offene Übergaben aus list-pipedrive-funding-handoffs fortsetzen. Bei Förderhöhe erste Zeile immer Voraussichtlich … Förderung mit EFH-Prozent beziehungsweise MFH-Eurobetrag, dann knappe Bonusaufteilung und kurze Erklärung. Keine Quellen-, Datei-, Seiten- oder Regelstandblöcke. Fehlende BzA oder fehlendes Antragsdatum blockieren eine vorläufige Angebotsschätzung nicht; den aktuellen geprüften Regelstand intern annehmen. Ungeklärte Boni offen lassen, bekannte Grundförderung trotzdem nennen. Meldebescheinigungen ohne Altersgrenze. Einkommensbonus nur bei ausdrücklichem Hinweis in TMB, Deal-Informationen einschließlich menschlicher Notizen oder unterschriebenem Angebot; sonst keine Steuerunterlagen verlangen und vollständige Fälle zur Beantragung weitergeben. Anhänge vollständig als lesbare, richtig benannte PDFs ablegen, KfW-Kontoinformationen im Text ebenfalls zuordnen. Erst nach verifiziertem Upload/Notiz und vollständiger Verarbeitung nach Posteingang/Fertig verschieben. Temporäre lokale Kopien nach bestätigter Ersatzablage entfernen. Keine Fachsystem-Dateien löschen. KfW-Zusage eindeutig prüfen, alle Deal-Labels entfernen, Gewonnen speichern und den nachgelagerten Terminierungsdeal rücklesen. Fehlgeschlagene technische Schritte reparieren und beim ersten offenen Schritt fortsetzen; kein Erfolg ohne Beleg.\n\nKonfiguration dieses Laufs: fundingRun=${JSON.stringify(fundingRun)}. Sie wird im request.json gespeichert. CLI-Scan: node local-mac-helper/cli.mjs scan-funding-mailbox --funding-run <absolute-request.json>. Bereits vollständig gelesene Seiten nicht neu anfordern; offenen Cursor übernehmen.`, acceptanceCriteria: [...taskDefinition.acceptanceCriteria, 'Kunde im An-Feld, verifizierter Vertriebspartner im CC; unklare Empfänger oder Inhalte werden nicht versandt.', 'Lesbare PDF-Ablage und Notizen mit (Notiz von Nadine) sind nachgewiesen; keine Altersgrenze für Meldebescheinigungen.', 'Telefonnummer, Kunden-E-Mail, Anlage und Auftragsnummer wurden in ihren kanonischen CRM-Feldern rückgelesen; fehlende Werte bleiben ein offener Einzelfall.', 'Keine Förderhöhen-Notiz vor dokumentierter Vollständigkeit und bestätigtem Übergang zur Beantragung; offene Notizablage nur mit gespeichertem eigenem Übergabebeleg fortsetzen.'] };
  }

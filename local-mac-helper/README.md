...
Der Geräteagent fragt IVA-Core alle 15 Sekunden über HTTPS nach einem eng freigegebenen Befehl. Der dauerhaft laufende Runner liegt lokal unter `~/Library/Application Support/IVA Mac Helper/device-agent/` und aktualisiert sich kontrolliert aus dem verbindlichen iCloud-Workspace. Förderläufe halten Rechner und Bildschirm bis zum protokollierten Ende wach. Nach Abschluss sämtlicher laufender und unmittelbar wartender Bildschirmaufträge darf IVA das Display zwischen 22:00 und 07:00 Uhr einmal ausschalten. Eine kurze Nachlaufzeit prüft bis zu 60 Sekunden auf ruhige Eingaben; neue Nutzeraktivität verhindert die Abschaltung. Gemeinsame Auftragsbelege verhindern Aus-/Einschalten zwischen Arbeitsschritten. Tagsüber erfolgt keine erzwungene Abschaltung. Der Mac bleibt für Hintergrundaufträge wach. Das Aufwecken des Displays entsperrt keine macOS-Passwortsperre; deren Einstellungen werden nicht automatisch verändert. Die Installation gilt erst nach zwei zeitversetzten, fortlaufenden Railway-Heartbeats als erfolgreich. Der iMac öffnet keinen eingehenden Port und aktiviert weder SSH noch macOS Remote Login. Das Gerätetoken liegt im macOS-Schlüsselbund unter `de.iva.device-agent`; Railway erhält denselben Wert ausschließlich als Secret `IMAC_DEVICE_TOKEN`.

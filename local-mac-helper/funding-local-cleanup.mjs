...
export async function cleanupCompletedFundingReview(messageFingerprint) {
  ...
  if (completion.pipedriveUploadVerified !== true || completion.pendingManualReview !== false) {
  ...
  if (review.localCleanup?.status === 'complete' && !review.downloaded?.directory) {
    await access(expectedDirectory).then(() => { throw new Error('Der bereits bereinigte Förderordner ist wieder vorhanden und muss geprüft werden.'); }, error => { if (error.code !== 'ENOENT') throw error; });
    return { messageFingerprint: review.messageFingerprint, dealId: review.dealId, localFilesDeleted: true, alreadyCompleted: true, deletedScope: 'managed_local_funding_copies_only', emailDeleted: false, pipedriveFileDeleted: false };
  }
  ...
  const directory = await realpath(configuredDirectory).catch(error => { if (error.code !== 'ENOENT') throw error; return null; });
  if (directory && !isInside(root, directory)) throw new Error('Lokale Bereinigung gesperrt: Ziel liegt außerhalb von IVAs Förder-Arbeitsordner.');

  if (directory) await rm(directory, { recursive: true, force: false });
  await access(configuredDirectory).then(
...
export function fundingLocalCleanupPolicy() {
  return Object.freeze({
    ...
    requiresVerifiedOutbound: false,
    ...
    emptiesWholeUserTrash: true,
    wholeUserTrashCleanupExecutor: 'authorized-daily-worker',
  });
}

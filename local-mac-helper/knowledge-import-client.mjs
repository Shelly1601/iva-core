...
export async function completeKnowledgeImportFromManifest(importId, manifestPath, dependencies = {}) {
  ...
  return report(importId, {
    ...
    sourceEvidence: parsed.sourceEvidence && typeof parsed.sourceEvidence === 'object' ? {
      sourceUrl: String(parsed.sourceEvidence.sourceUrl || '').slice(0, 1800),
      sourceRead: parsed.sourceEvidence.sourceRead === true,
      mediaType: parsed.sourceEvidence.mediaType === 'video' ? 'video' : 'page',
      videoRead: parsed.sourceEvidence.videoRead === true,
      audioRead: parsed.sourceEvidence.audioRead === true,
      visualsRead: parsed.sourceEvidence.visualsRead === true,
      speechPresent: typeof parsed.sourceEvidence.speechPresent === 'boolean' ? parsed.sourceEvidence.speechPresent : null,
      method: String(parsed.sourceEvidence.method || '').slice(0, 160),
    } : undefined,

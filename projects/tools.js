...
export function projectSkill({projectId,getProject,readProjectFile,addProjectNote,connections}) {
  ...
  const tools={
    ...
    listCurrentProjectConnections:tool({description:'Zeigt ausschließlich die Anbindungen des aktuellen Projekts, fehlende Angaben und den letzten tatsächlichen Verbindungscheck. Gibt keine Zugangsdaten zurück.',parameters:z.object({}),execute:async()=>{await current();const snapshot=await connections.list(projectId);const items=snapshot.items||[];const configured=items.filter(item=>item.configured===true);return {projectId,connectionCount:items.length,configuredAccountCount:configured.length,connections:items,summary:configured.length?`${configured.length} Konto-Zugang/Zugänge in diesem Projekt eingerichtet; Verifizierung je Konto prüfen.`:items.length?'Kontodaten sind vorgemerkt, aber in diesem Projekt ist noch kein nutzbarer Konto-Zugang eingerichtet.':'In diesem Projekt sind keine Konto-Zugänge hinterlegt.'};}}),

...
function validatePayload(action, payload = {}) {
  ...
  if (action === 'knowledge.import.start') {
    ...
    const completionVersion = payload.completionVersion === undefined ? 1 : Number(payload.completionVersion);
    if (![1, 2].includes(completionVersion)) throw new Error('Unbekannte Wissensimport-Abschlussversion.');
    return {
      importId,
      entryId,
      title,
...
export async function completeDeviceCommand({ deviceId, commandId, leaseToken, ok, result = null, error = '', failureStage = '', agentMetadata = {} } = {}) {
  return transaction(async () => {
    ...
    if (retryPlanbarBeforeLaunch || retryKnowledgeImport || retryCustomerCare) {
      command.status = 'queued';
      command.retryAt = new Date(Date.now() + (retryCustomerCare || retryKnowledgeImport ? Math.min(300_000, 2_000 * 2 ** Math.min(command.attempts, 8)) : command.attempts * 15_000)).toISOString();
      command.failureStage = retryCustomerCare ? 'customer-care-resume' : retryKnowledgeImport ? 'automatic-recovery' : 'before_launch';

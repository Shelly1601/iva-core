...
const state = { entries: [], current: null, status: null, imports: [], capabilities: null, importPolling: false, categories: new Set(['Allgemein', 'Marketing', 'Sales', 'Versicherung']) };
...
function renderCategories() {
  for (const item of state.entries) if (item.category) state.categories.add(item.category);
  $('categoryOptions').innerHTML = [...state.categories].sort((a, b) => a.localeCompare(b, 'de')).map(value => `<option value="${esc(value)}"></option>`).join('');
}
...
function renderImports() {
  ...
  $('importConnection').textContent = ready ? 'Mac Mini bereit' : state.capabilities?.device?.online ? 'Mac Mini aktualisiert Verbindung' : 'Mac Mini verbindet sich';
...
async function encryptCredentials(metadata, credentials) {
  if (!globalThis.crypto?.subtle || !metadata?.publicKey) throw new Error('Die sichere Mac Mini-Verschlüsselung ist noch nicht bereit.');
...
function resetForm() {
  state.current = null; $('entryForm').reset(); $('sourceOwner').value = 'public-reference'; $('kind').value = 'knowledge'; $('formTitle').textContent = 'Link rein. Wissen aufnehmen.'; $('deleteEntry').hidden = true; $('formState').textContent = ''; $('importState').textContent = ''; $('detail').hidden = true;
  $('contentOptions').open = false; $('importOptions').open = false; syncCredentialFields(); $('startImport').textContent = 'Aufnehmen';
...
function fillForm(item) {
  ...
  if (item.status === 'ready') { const link = document.createElement('a'); link.className = 'btn primary'; link.textContent = 'Aus diesem Wissen ein Produkt erstellen'; link.href = '/product-creator?knowledgeId=' + encodeURIComponent(item.id); $('detail').append(link); }
  $('contentOptions').open = true; $('startImport').textContent = 'Änderungen speichern';
  $('openDocument')?.addEventListener('click', () => openDocument(item.id));
}
function payload() { return { title: $('title').value.trim(), documentName: $('document').files[0]?.name, kind: $('kind').value, category: $('category').value.trim(), sourceUrl: $('sourceUrl').value.trim(), sourceOwner: $('sourceOwner').value, tags: tagList($('tags').value), content: $('content').value.trim(), notes: $('notes').value.trim() }; }
function syncCredentialFields() {
  const enabled = $('useCredentials').checked;
  $('credentialFields').disabled = !enabled; $('credentialFields').hidden = !enabled;
  if (!enabled) { $('loginUsername').value = ''; $('loginPassword').value = ''; $('loginTotp').value = ''; }
}
async function optionalCredentials() {
  if (!$('useCredentials').checked) return null;
  const username = $('loginUsername').value.trim(), password = $('loginPassword').value, totp = $('loginTotp').value.trim();
  if (!username && !password && !totp) return null;
  if (!username || !password) throw new Error('Für den optionalen Zugang Benutzername und Passwort ergänzen oder „Zugangsdaten ergänzen“ ausschalten.');
  if (!state.capabilities?.credentialEnvelope?.publicKey) await loadImports();
  return encryptCredentials(state.capabilities?.credentialEnvelope, { username, password, totp });
}
...
async function loadAll(query = '') {
  ...
  state.status = status; state.entries = result.entries || []; state.imports = imports.imports || []; state.capabilities = capabilities; renderMetrics(); renderEntries(); renderCategories(); renderImports();
}
...
async function startImport() {
  ...
  if (!sourceUrl) return notify('Bitte einen Link einfügen.', 'error');
  setBusy(button, true, 'Wird sicher übergeben …'); $('importState').textContent = '';
  try {
    const credentialEnvelope = await optionalCredentials();
    const result = await api('/api/knowledge/imports', { method: 'POST', body: JSON.stringify({
      title,
      kind: $('kind').value === 'course' ? 'course' : 'link',
      sourceUrl,
    ...
    resetForm(); $('category').value = category;
    $('importState').textContent = 'Link aufgenommen.';
    notify(result.import.status === 'failed' ? 'Der Link ist gespeichert. Der Start ist noch offen; nutze „Fortsetzen“ beim gespeicherten Import.' : 'Link und Kategorie sind gespeichert. IVA übernimmt den Import; den Fortschritt siehst du hier.', result.import.status === 'failed' ? '' : 'good');
    state.imports = [result.import, ...state.imports.filter(item => item.id !== result.import.id)]; renderImports();
    try { await loadAll($('searchInput').value.trim()); }
    catch { notify('Der Import ist gespeichert. Die Fortschrittsanzeige wird beim nächsten Abruf aktualisiert.', 'good'); }
...
async function saveEntry(event) {
  ...
  if (!state.current && $('sourceUrl').value.trim() && !$('content').value.trim() && !file) return startImport();
  if (!state.current && !$('sourceUrl').value.trim() && !$('content').value.trim() && !file) return notify('Füge einen Link, Text oder eine Datei hinzu.', 'error');
  const button = event.submitter || $('startImport'); setBusy(button, true, 'IVA nimmt es auf …'); $('formState').textContent = '';
  try {
    ...
    state.current = item; $('deleteEntry').hidden = false;
    ...
    fillForm(item); $('formState').textContent = 'Gespeichert.'; notify(item.status === 'ready' ? 'Das Wissen ist gespeichert und für IVA verfügbar.' : 'Die Quelle ist vorgemerkt. Ergänze noch Lernmaterial, damit IVA sie verwenden kann.', item.status === 'ready' ? 'good' : '');
    try { await loadAll($('searchInput').value.trim()); }
    catch { notify('Der Inhalt ist gespeichert. Die Übersicht wird beim nächsten Abruf aktualisiert.', 'good'); }
  } catch (error) { $('formState').textContent = error.message; notify(error.message, 'error'); } finally { setBusy(button, false); if (state.current) button.textContent = 'Änderungen speichern'; }
}
...
$('entryForm').addEventListener('submit', saveEntry); $('newEntry').addEventListener('click', () => { resetForm(); $('sourceUrl').focus(); }); $('deleteEntry').addEventListener('click', removeEntry);
$('useCredentials').addEventListener('change', syncCredentialFields);

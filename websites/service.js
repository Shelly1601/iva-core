...
export function createWebsiteService({ dataDir, getProject, listProjects, env = process.env, compile = compileWebsite, generate = generateWebsite, importUrl = importWebsiteUrl, referenceReader = readWebsiteReference, githubFactory = createGitHubWebsiteConnector, fetchImpl = fetch, dnsCname = resolveCname, jobTimeoutMs = 6 * 60_000 } = {}) {
  ...
  async function importSite(p, s, input, locked = false, context) {
    const run = async () => {
      ...
      if (input.kind === 'github') {
        const result = await (await github()).importRepository({ repository: input.repository });
  ...
  async function exportGitHub(p, s, input = {}, locked = false, context) {
    const run = async () => {
      ...
      try {
        ...
        checkOperation(context);
        const pending = Promise.resolve().then(() => connector.exportRepository({ name: slug(input.name || current.name), description: `Website ${current.name} · IVA Website Studio`, files: revision.files, private: true, ...(existing ? { repository: existing.repository, expectedHead: existing.commitSha } : {}) }));
        started = true;
  ...
  async function chat(p, s, input) {
    ...
    const context = { active: true, controller: new AbortController(), jobId };
  ...
  return {
    ...
    listProjects: async () => (await listProjects()).map(({ id, name, description }) => ({ id, name, description })),
    create: async input => store.create(input.projectId, input),
    list: p => store.list(p),

...
- Pipedrive wird ausschließlich über die offizielle, auf Railway hinterlegte API im IVA-Core-Hintergrund gelesen. Deallisten, Details, Notizen und Dateien dürfen nicht durch Öffnen, Schließen oder Auslesen von Chrome-Tabs beschafft werden. Die geprüften iMac-Befehle `scan-funding-board`, `read-pipedrive-deal` und `download-pipedrive-files` verwenden den gesicherten Gerätekanal und geben keine API-Schlüssel oder Download-URLs aus.

...
function normalizedMeeting(input = {}) {
  ...
  return {
    ...
    coachSessionId: cleanText(meeting.coachSessionId, 160),
    segments: (Array.isArray(meeting.segments) ? meeting.segments : []).slice(0, 5000).map(row => ({ id: cleanText(row?.id, 180), speakerId: cleanText(row?.speakerId, 180), start: Math.max(0, Number(row?.start) || 0), end: Math.max(0, Number(row?.end) || 0), text: cleanText(row?.text, 12000) })),
    speakers: (Array.isArray(meeting.speakers) ? meeting.speakers : []).slice(0, 500).map(row => ({ id: cleanText(row?.id, 180), clipId: cleanText(row?.clipId, 120), label: cleanText(row?.label, 180), name: cleanText(row?.name, 100), confirmed: row?.confirmed === true })),
